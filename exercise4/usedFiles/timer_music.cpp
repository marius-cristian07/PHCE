...
// Internal linkage: only this translation unit can see these names.
namespace
{
...
// Called from the timer interrupt every time the timer fires.
bool toggle_callback(repeating_timer_t* t)
{
    ...
    toggler->level = !toggler->level;
    gpio_put(toggler->pin, toggler->level);
    ++toggler->toggles;
...
int main()
{
    ...
    while (true)
    {
        // TODO 3: play every note in MELODY, in order, with play_note().
        for (const Note& note : MELODY)
        {
            play_note(note, buzzer);
        }
